...
2. Read `CLAUDE.md` for project conventions that will constrain the plan.
...
- A requirement that names a behavior but not the trigger ("the inventory updates" — when?).
...
- Conform to `architecture.md` (and any `<Feature>.architecture.md`): follow its Guidelines, Structure, and Decisions; place new code where the architecture says it belongs; don't propose patterns or infrastructure it rules out. Where a plan decision is shaped by the architecture, cite it in a rationale note (e.g. `// arch: cross-system access via events per architecture.md D2, not a singleton`).
- Include short rationale notes pointing back to the spec where a non-obvious decision was made. Format: `// R3: chose event over direct call because spec requires loose coupling between inventory and UI`.
