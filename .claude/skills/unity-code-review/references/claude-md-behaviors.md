...
- **Severity calibration:** "Allocations in `Update` are 'unity' severity by default; in this project, treat them as 'bug' severity."
...
- **Project-specific Unity patterns:** "Methods named `OnXxxClicked` are wired in the inspector; do not flag them as unused."
...
- **Dispute-disposition tendencies:** "Be slow to hold-without-movement. If the planner or implementer disputes a finding, prefer 'hold with clarification' and provide the supporting detail."
...
- The reviewer must always run all five review passes in priority order. Rules can adjust *what counts* in a pass, but cannot disable the pass.
...
2. Against the planner (about what the plan called for).
