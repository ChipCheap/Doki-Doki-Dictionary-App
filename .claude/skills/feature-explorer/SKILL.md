...
name: explorer
description: >
  Explores and fleshes out a new design or feature before specification.
  Draws the user out through batched critical questions, hunts for edge
  cases and failure modes, proposes product-standard defaults (game or
  app), and closes gaps. Produces a framework file next to CLAUDE.md that
  the specifier skills consume as input and as boundaries the spec must
  not break. Supports a project-wide framework.md plus feature-level
  <Feature>.framework.md files, and keeps the two consistent.
---
...
explore → specify → plan → implement → review
...
implementation must stay inside." The specifier takes it from there.
...
## Routing: which product standards, which specifier

Detection only affects the standards you suggest and the specifier you
hand off to.

**R1 — Unity context?** An `Assets/` or `ProjectSettings/` folder, any
`.unity` files, or `## Project Context` naming Unity → treat as a **game**
for standards, hand off to `unity-specifier` at the end.
...
**devil's advocate** (hunt edge cases and conflicts), **domain expert**
...
When the file is written, hand off to the specifier:

Design framework written to <file>.
It inherits the boundaries in framework.md<if present>.

Next step is specification. Trigger the specifier when ready — it'll read
this framework (and framework.md) as its input and as the boundaries the
spec must respect. This is a <Unity / non-Unity> project, so
<unity-specifier / the generic specifier> is the right one.
