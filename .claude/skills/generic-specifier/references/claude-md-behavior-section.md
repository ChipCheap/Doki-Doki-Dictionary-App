...
- **Add** behaviors — "always ask about migration compatibility for schema changes" (specifier).
...
- **Constrain** behaviors — "don't propose global state patterns; this project uses dependency injection" (planner).
...
- "Update the specifier's behavior to always ask about migration compatibility."
...
- Always ask about migration compatibility for schema changes.
...
Same pattern in reverse. The user says "remove the rule about migration compatibility" or "change the rule about rationale notes to..." — the skill proposes the diff, shows it, and waits for confirmation.
