...
- Added step S4 to address finding F2 (null check on the returned user object).
...
### S1: <short title>
**Covers:** R1
**Files:** src/services/auth/auth-service.ts (new)
**Description:** <one paragraph: what to do, mechanically>
**Rationale:** <if a non-obvious choice was made — e.g. "R1 says 'fast lookup'; chose Map over Array for O(1) access">

### S2: <short title>
**Covers:** R2
**Files:** src/ui/auth/login-form.tsx (modify)
...
### H1: <short title>
**Justification:** Scaffolding for S3 — refactors the existing request router to make room for the new auth middleware.
**Files:** src/middleware/request-router.ts (modify)
**Description:** <one paragraph>
...
- **R1 satisfied when:** <observable criterion, e.g. "AuthService.getUser returns the correct user for any registered id in O(1) time">
- **R2 satisfied when:** ...
```

## Style notes

- **Concrete file paths, not abstract descriptions.** "Modify the auth thing" is not actionable; "Modify src/services/auth/auth-service.ts" is.
