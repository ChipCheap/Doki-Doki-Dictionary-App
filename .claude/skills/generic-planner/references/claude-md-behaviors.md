...
- **Pattern prohibitions:** "Never propose global mutable singletons for shared state."
...
The planner's own procedure already treats `CLAUDE.md` as additive (alongside generic language conventions) — that's separate from the `## Skill Behaviors` section.
