...
**Before:**
> R3: The user session is revoked when the user signs out.

**After:**
> R3: The user session is revoked when the user signs out, where "signs out" means the user clicks the explicit Sign Out button or their session token expires. The revocation is synchronous within the same request.
...
**C1 (applies to R3):** "Signs out" means the user clicks the explicit Sign Out button or their session token expires. Revocation happens synchronously within the same request.

**C2 (applies to R5):** Rate-limiting applies only to anonymous requests. Authenticated requests bypass the rate limiter regardless of identity.
```
...
R3 changed from:
> The user session is revoked when the user signs out.
to:
> The user session is revoked when the user signs out, where "signs out" means...

Added C2:
> Rate-limiting applies only to anonymous requests...
