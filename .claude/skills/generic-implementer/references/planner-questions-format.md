...
- "Plan step S3 says fire an event when data refreshes — does 'refresh' mean only newly-fetched data, or also stale data being replaced from cache?"
...
- "S2 says modify `UserView` but `UserView` doesn't exist in the codebase. Is this a missing earlier step, a rename, or a typo?"
...
- Style preferences (CLAUDE.md authoritative; if CLAUDE.md is silent, follow conventions appropriate to the project's language as recorded in Project Context).
