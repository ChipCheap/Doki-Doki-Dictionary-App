...
description: Implement a plan in a Unity C# project, with technical clarification before coding, immediate user check-ins on architectural ambiguity, post-implementation plan-coverage verification, and disciplined cleanup of unused code within the changeset. Use this skill whenever an implementer sub-agent is asked to execute a plan in a Unity project — typically spawned by the unity-orchestrator skill. Trigger on prompts like "implement this plan", "execute Plan.md", "you are the implementer for…", or any context where a Plan.md exists in a feature folder and the next step is to turn it into code. The output is the code itself on disk plus a structured summary the orchestrator can route onward.
---

# Unity Implementer

An implementer sub-agent for the implementation cycle. Reads `Plan.md` and turns it into working Unity C# code. Owns four responsibilities:
...
This skill is invoked as a sub-agent by the `unity-orchestrator` skill. It can also be run standalone — the procedure is the same.
...
- "Used `UnityEvent` instead of plain C# event because S4 requires inspector-wiring."
- "Cached `Camera.main` in `Awake` rather than per-call per CLAUDE.md's no-`Camera.main`-in-hot-paths rule."
- "Stored the lookup as `Dictionary<int, Item>` rather than `List<Item>` because R3 requires O(1) access."
...
**For step S4:**
*Made the prefab variant inherit from the base prefab rather than duplicating, since CLAUDE.md prohibits prefab duplication for inventory items.
```
...
```markdown
### S2: Wire inventory updates to UI

**Covers:** R2
**Files:** Assets/Scripts/UI/InventoryView.cs (modify)
**Description:** Subscribe InventoryView to InventoryService's OnChanged event*
and refresh the visible slot list when fired*.

**Technical note:** *Used UnityEvent instead of plain C# event because S4
requires inspector-wiring.
**Technical note:** *Refresh debounced to one rebuild per frame to avoid
multiple Canvas rebuilds when a stack of items is added in a single tick.
```
...
4. **Implement with Unity idioms.** Follow Unity best practices (caching, serialization conventions, lifecycle hooks, reflection-driven members) per `references/unity-implementation.md`.
...
2. **Apply Unity reflection awareness.** Many Unity members look unused to a static reader but are wired via the engine. See `references/unused-code-rules.md` for the full checklist of reflection-driven patterns to leave alone.
...
- "The reviewer flags this field as unused — it's wired via UnityEvent in the InventoryUI prefab."
...
Common rule types for the implementer: project-specific cull aggressiveness (more conservative or more permissive within strict bounds), preferred Unity idioms, mandatory technical-note categories, project-specific architectural-pause triggers.
...
- `references/unused-code-rules.md` — what counts as unused, the Unity reflection-driven exemption list, the TODO format, and what to never cull.
- `references/unity-implementation.md` — Unity idioms the implementer should follow (lifecycle, caching, serialization, reflection-aware patterns).
