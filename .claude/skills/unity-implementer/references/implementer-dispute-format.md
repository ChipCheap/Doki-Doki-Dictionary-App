...
- The finding flags a member as unused when it's reflection-driven (inspector-wired UnityEvent, `[SerializeField]`, `SendMessage` target, animation event callback).
...
- File and line: <Assets/Scripts/Player/PlayerController.cs:88>
- Caller(s) of the method in question: <list them, with file:line each>
- Or — wiring source: "Inspector-wired in Assets/Prefabs/InventoryUI.prefab via UnityEvent on the OnSubmit field of the inventory button"
