...
**Language and stack:** Every sub-agent prompt must include the project's language and stack so the sub-agent operates in the right idioms. Pull these from `CLAUDE.md`'s `## Project Context` section. Include them in the prompt body, just under the project conventions block. Example: "This project is TypeScript on Next.js, runtime Node 20, tests with Jest." The CLAUDE.md content itself is also passed, but a one-line summary at the top of each prompt makes the language explicit and prevents the sub-agent from having to scan.
...
```
You are the planner for feature "<feature_name>" in this project.
...
```
You are the implementer for feature "<feature_name>" in this project.
...
```
You are the reviewer for feature "<feature_name>" in this project.
Use the generic-code-review skill.
...
## Your task
Run the generic-code-review skill on the changes. Return findings in the skill's standard format, or a clean summary if no findings.
```
