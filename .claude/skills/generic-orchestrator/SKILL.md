...
description: Coordinate a planner → implementer → reviewer loop of sub-agents to implement a specified feature in any software project, with token-budget-aware checkpointing and escalation on anomalies. Use this skill whenever a feature folder containing a Spec.md exists alongside CLAUDE.md and the user is moving toward implementation — typically signaled by phrases like "let's implement this", "start the build", "kick off the cycle", or by the user pointing at a feature folder. Also use when the user says "continue with the last feature", "resume", or references a Handoff.md. Defers to unity-orchestrator if Unity context is present (Assets/ folder or Project Context names Unity). Orchestrated mode is the default when preconditions are met. Do NOT use this skill if the user explicitly opts out with phrases like "implement normally", "without orchestration", "skip the orchestration", or similar — in those cases hand control back to default planning behavior.
---
...
### 0. Preliminary check: preconditions, routing, and opt-out

Before doing anything else, verify that orchestrated mode is appropriate AND that this is the right skill for the project.
...
**Step 0b — Route Unity projects to unity-orchestrator.**
...
If either is true → defer to `unity-orchestrator`:

```
This looks like a Unity project. The unity-orchestrator skill is a better fit —
it has Unity-aware sub-agent prompts. Switching to that.
```

Do not proceed with this skill.

**Step 0c — Verify the folder structure.**
...
**Step 0d — Verify Project Context is recorded.**

Check `CLAUDE.md` for a `## Project Context` section. If it doesn't exist, the project's language/stack hasn't been recorded — downstream sub-agents won't know what to do. Stop and tell the user:

```
I can't find a Project Context section in CLAUDE.md. The sub-agents need to know
what language and stack the project uses. Run the generic-specifier skill first,
or add a Project Context section to CLAUDE.md manually.
```

Do not invent the language from file extensions or other signals.

**Step 0e — Proceed.**

If preconditions are met, no opt-out was detected, and Project Context is in place, proceed to step 1 in orchestrated mode. Do not ask the user to confirm; the default is orchestrated mode.
...
- **Do no domain work yourself.** The orchestrator does not plan, implement, or review. If you find yourself reasoning about the code, stop and spawn the appropriate sub-agent.
