...
|---|---|---|---|
| ~~O1~~ | **Font files.** Hookup done — `src/ui/fonts.ts` declares all ten faces with the base path applied, and `public/fonts/README.md` carries the download instructions. **Awaiting the `.woff2` files only.** H2 calls for self-hosted `.woff2` for the five curated faces; binaries cannot be authored here. | Font stacks name all five with system fallbacks. | The settings picker works; text renders in a fallback face. Only the Vietnamese-coverage guarantee is unproven until the real files land. |
| ~~O2~~ | **PWA icons.** Location fixed: `public/icon-192.png` and `public/icon-512.png`, already referenced by the manifest. **Awaiting the files only.** `icon-192.png` and `icon-512.png`, same problem — and the 512 needs its subject inside the maskable safe zone, which is a design call. | Referenced by the manifest, files absent. | Dev and build run without them; only installability is affected. |
| O3 | **Seed pack content is authored, not sourced.** Swap-in point: put real files in `public/packs/`, then edit `src/dictionary/catalog.ts` (paths + drop `provisional`). Nothing else knows the filenames. Both packs are labelled `TEST FIXTURE — not sourced data`. | 20 words per language, chosen to exercise the hard paths. | It proves the format, which is its whole job. It must not be trusted for actual study — especially the Vietnamese tones — and is replaced when the real pipeline exists. |
| ~~O4~~ | **DONE** — `.gitattributes` added with `* text=auto eol=lf` plus binary rules. Original note: Windows is normalising LF→CRLF on commit. | Absent. | Harmless locally; causes spurious whole-file diffs if the repo is ever cloned on Linux. One line fixes it permanently. |
| ~~O5~~ | **DONE** — reviewed, see `CODEMAP.md`. Original note: The user wants to read the `dictionary` and `progress` code once the slice is complete. | Written, typechecked, tests green. | Nothing depends on the review; it is a read-through, not a gate. |
